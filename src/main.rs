...
use egui::{Context, CornerRadius, Mesh, Rangef, Sense, Stroke, Ui, Vec2};
use crate::egui_gauge::Gauge;
...
impl App {
    pub fn new<'a>(cc: &'a eframe::CreationContext<'a>, rbr_data_handle: Arc<ArcSwapAny<Arc<RbrHeader>>>, app_data_handle: Arc<ArcSwapAny<Arc<AppData>>>) -> Self {
        ...
        pedal_graph.add_function(Color32::GREEN, 1., 0., 100.); // throttle
        pedal_graph.add_function(Color32::RED, 1., 0., 100.); // brake
        pedal_graph.add_function(Color32::ORANGE, 1., 0., 100.); // clutch
        pedal_graph.add_function(Color32::YELLOW, 1., 0., 100.); // handbrake
        pedal_graph.add_function(Color32::GRAY, 1., 0., 6.); // gear
                ...
                egui::ScrollArea::vertical()
                    ...
                    .show(ui, |ui| {
                        ui.vertical_centered(|ui| {
                            ...
                            ui.add(egui::Slider::new(&mut *self.zoom.borrow_mut(), 0.05..=4.0).text("Zoom"));
                            ...
                            ui.add_space(10.);
                            ui.heading("Time");
                            let time_text = if is_valid {
                                let time = rbr_header.telemetry.get_time();
                                let formatted = format!("{:02} : {:02}", &time.minutes, &time.seconds);
                                formatted
                            } else {
                                "00 : 00".to_owned()
                            };
                            ...
                            ui.add_space(10.);
                            ui.heading("Steering");
                            ui.add(egui::Slider::new(&mut self.max_steering_angle, 450.0..=1200.).text("Max steering Angle"));
                            egui::Frame::dark_canvas(ui.style())
                                .show(ui, |ui| {
                                    ...
                                    let mut angle: f32 = 0.;
                                    let mut steering_normalized: f32 = 0.;
                                    if is_valid {
                                        steering_normalized = rbr_header.telemetry.control.steering;
                                        angle = -1. * steering_normalized * self.max_steering_angle.to_radians();
                                    }
                                    ...
                                    let fill_width = rect_width * steering_normalized.abs();
                                    if steering_normalized < 0.0 {
                                        painter.rect_filled(
                            ...
                            egui::Frame::dark_canvas(ui.style())
                                .show(ui, |ui| {
                                    ...
                                    let mut rpm = 0.;
                                    if is_valid {
                                        speed = rbr_header.telemetry.car.speed.floor().max(0.);
                                        rpm = rbr_header.telemetry.car.engine.rpm.floor().max(0.);
                                    }


                                    ui.heading("Speed [km/s]");
                                    ui.add(Gauge::new(
                                        speed, 0.0..=200., 200., Color32::LIGHT_GREEN
                                    ));
                                    ui.heading("RPM");
                                    ui.add(Gauge::new(
                                        rpm, 0.0..=9000., 200., Color32::LIGHT_RED
                                    ));
                                });
                        });
                    });
    ...
    pub fn update_state(&mut self, _delta_time: f32) {
        ...
        let transformations = vec![
            Transform {
                ...
                rotation: Quaternion::new_rotation(speed * t, Vector::new(1., 0.9, 0.3)),
